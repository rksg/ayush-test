...
const configuration: IntentConfigurationConfig = {
  label: defineMessage({ defaultMessage: 'Background Scan Timer (5 GHz)' }),
  valueFormatter: formatter('durationFormat')
}

const useValuesText = createUseValuesText({
  action: defineMessage({ defaultMessage: '5 GHz radio setting for {scope} has "Background Scan Timer" set as {currentValue}. Recommended setting for "Background Scan Timer" is {recommendedValue} to effectively use "Background Scan" feature.' }),
  ...
  ` }),
  inactive: defineMessage({ defaultMessage: 'When activated, this AIOps Intent takes over the automatic configuration Background Scan Timer on 5 GHz radio in the network.' })
})
