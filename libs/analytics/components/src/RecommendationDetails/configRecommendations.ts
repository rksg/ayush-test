...
const configs: Record<string, RecommendationConfig> = {
  ...
  'c-bandbalancing-enable': bandbalancingEnable,
  'c-bandbalancing-enable-below-61': bandbalancingEnable,
  'c-txpower-same': {
  ...
  'c-crrm-channel24g-auto': {
    ...
    appliedReasonText: defineMessage({ defaultMessage: 'AI-Driven Cloud RRM will constantly monitor the network, and adjust the channel plan and bandwidth when necessary to minimize co-channel interference. These changes, if any, will be indicated by the Key Performance Indicators. The number of interfering links may also fluctuate, depending on any changes in the network, configurations and/or rogue AP activities.' }),
...
export default merge(codes, configs) as unknown as Record<keyof typeof codes, RecommendationConfig & CodeInfo>
