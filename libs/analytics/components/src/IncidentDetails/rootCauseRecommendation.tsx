...
export const rootCauseRecommendationMap = {
  'assoc': {
    CCD_REASON_NOT_AUTHED: {
      rootCauses: defineMessage({ defaultMessage: '<p>Clients are failing to connect during the 802.11 open authentication, but the exact reason for the failures is unclear. This maybe caused due to TCM Transient Client Management , SmartRoam or ACL (Access Control List) connection failures if configured on your network.</p>' }),
      recommendations: commonRecommendations
    },
    CCD_REASON_NOT_ASSOCED: {
      rootCauses: defineMessage({ defaultMessage: '<p>Clients are failing to connect during the association stage, but the exact reason for the failures is unclear. This maybe caused due to TCM Transient Client Management , SmartRoam or ACL (Access Control List) connection failures if configured on your network.</p>' }),
      recommendations: commonRecommendations
    },
    CCD_REASON_AUTH_ALG: {
