...
export function useTenantLink (to: To, tenantType: TenantType = 't') {
  ...
  const rpath = resolvePath(path, isRa ? MLISA_BASE_PATH : `/${tenantId}/${tenantType}`)
  if (isRa && rpath.pathname.includes('/analytics/')) {
    rpath.pathname = rpath.pathname.replace('/analytics', '')
  }
  return rpath
}
