...
describe('Venue Client Admission Control', () => {
  ...
  it('should render correctly', async () => {
    ...
    expect(await screen.findByTestId('client-admission-control-max-radio-load-24g')).toBeVisible()
  ...
  it('should handle turn On/Off switch buttons changed', async () => {
    ...
    expect(await screen.findByTestId('client-admission-control-max-radio-load-50g')).toBeVisible()
    ...
    render(
      <Provider>
        <VenueEditContext.Provider value={{
          ...
          <Form>
            <ClientAdmissionControlSettings isLoadOrBandBalaningEnabled={true} />
          </Form>
        </VenueEditContext.Provider>
      </Provider>, {
        route: { params, path: '/:tenantId/venues/:venueId/edit/:activeTab/:activeSubTab' }
