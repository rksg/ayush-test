...
export function PropertyUnitDrawer (props: PropertyUnitDrawerProps) {
  ...
  const dpskPassphraseLengthEnforced = useIsSplitOn(Features.DPSK_PASSPHRASE_LENGTH_ENFORCEMENT)
  ...
  const [dpskPoolId, setDpskPoolId] = useState<string|undefined>(undefined)
  ...
  useEffect(() => {
    if (!propertyConfigsQuery.isLoading && propertyConfigsQuery.data) {
      ...
      getPersonaGroupById({ params: { groupId } })
        .then(result => {
          setWithPin(!!result.data?.personalIdentityNetworkId)
          setDpskPoolId(result.data?.dpskPoolId)
        })
    }
              ...
              label={
                <>
                  ...
                  <Tooltip.Question
                    ...
                    // eslint-disable-next-line max-len
                      defaultMessage={'If empty, passphrase will be generated by the system.'}
                    />}
                  />
                </>
              }
              rules={[
                { min: dpskPassphraseLengthEnforced ? dpskData?.passphraseLength : 8 },
                { max: 63 },
                { validator: (_, value) => trailingNorLeadingSpaces(value) }
              ]}
              children={<Input />}
