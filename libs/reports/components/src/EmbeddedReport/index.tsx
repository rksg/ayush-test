...
export const getSupersetRlsClause = (
  ...
  if (radioBands?.length && isApReport && !isRadioBandDisabled) {
    radioBandClause = ` "band" in (${radioBands
  ...
  if (paths?.length) {
    ...
    paths.forEach((path) => {
      if (path.length === 2) {
        if (path[1].type === 'zone') zoneIds.push(`'${path[1].name}'`)
...
export function EmbeddedReport (props: ReportProps) {
  ...
  const [dashboardEmbeddedId, setDashboardEmbeddedId] = useState<string | null>(
    null
  )
  const {
    filters: { paths, bands }
  } = useReportsFilter()
  const { networkClause, radioBandClause } = getSupersetRlsClause(
    reportName,
    paths,
    bands as RadioBand[]
  )
