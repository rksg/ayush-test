  ...
  const data = [
    {
      ...
      networkDevices: apsCount > 0
        ? <UI.CellWrapper>
          ...
          <UI.TotalCount onClick={
            () => onNavigate && onNavigate({ venueId: undefined, path: 'devices/wifi' })}>
            {apsCount}
          </UI.TotalCount>
        </UI.CellWrapper>
        : <UI.TextWrapper>
      ...
      clients: clientsCount && clientsCount > 0
        ? <UI.CellWrapper>
          <UI.TotalCount onClick={
            () => onNavigate && onNavigate({ venueId: undefined, path: 'users/wifi/clients' })}>
            {clientsCount}
          </UI.TotalCount>
        </UI.CellWrapper>
        : <UI.TextWrapper>
    ...
    },
    {
      ...
      networkDevices: switchesCount > 0
        ? <UI.CellWrapper>
          <StackedBarChart
            ...
            {...commonProps} />
          <UI.TotalCount onClick={
            () => onNavigate && onNavigate({ venueId: undefined, path: 'devices/switch' })}>
            {switchesCount}
          </UI.TotalCount>
        </UI.CellWrapper>
        : <UI.TextWrapper>
      ...
      clients: switchClientsCount && switchClientsCount > 0
        ? <UI.CellWrapper>
          <UI.TotalCount onClick={
            () => onNavigate && onNavigate({ venueId: undefined, path: 'users/switch/clients' })}>
            {switchClientsCount}
          </UI.TotalCount>
        </UI.CellWrapper>
        : <UI.TextWrapper>
