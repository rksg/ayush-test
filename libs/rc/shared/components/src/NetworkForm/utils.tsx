...
interface asyncProcessTaskProps {
  promise: promiseTask | promiseTask[],
  condition: () => boolean
}

export const useAsyncProcessManager = () => {
  const [task, setTasks] = useState<asyncProcessTaskProps[]>([])

  // eslint-disable-next-line max-len
  const addTask = (promise: promiseTask | promiseTask[], condition: () => boolean = () => true) => {
    setTasks([...task, { promise, condition }])
  }

  const runAllTasks = async () => {
    return await Promise.allSettled(task.map(async ({ promise, condition }) => {
      if (condition()) {
        if (Array.isArray(promise)) {
          const sequentialPromises = []
          for (const p of promise) {
            sequentialPromises.push(await p)
          }
  ...
  return { addTask, runAllTasks }
}

export class AsyncProcessManager {
  private tasks: asyncProcessTaskProps[] = []

  // eslint-disable-next-line max-len
  addTask (promise: promiseTask | promiseTask[], condition: () => boolean = () => true) {
    this.tasks.push({ promise, condition })
  }

  async runAllTasks () {
    return await Promise.allSettled(this.tasks.map(async ({ promise, condition }) => {
      if (condition()) {
        if (Array.isArray(promise)) {
          const sequentialPromises = []
          for (const p of promise) {
