...
export const useEdgeClusterActions = () => {
  ...
  const shutdown = (data: EdgeStatus[], callback?: () => void) => {
    showActionModal({
      ...
      title: $t(
        {
          defaultMessage: `Graceful shutdown "{count, plural,
            one {{name}}
            other {{count} SmartEdges}
          }"?`
        }, { count: data.length, name: data[0].name }
      ...
      content: $t({
        defaultMessage: `Graceful shutdown will safely terminate all ongoing operations on 
      ...
      customContent: {
        ...
        buttons: [{
          ...
          text: $t({ defaultMessage: 'Graceful Shutdown' }),
          type: 'primary',
          key: 'ok',
          closeAfterAction: true,
          handler: () => {
