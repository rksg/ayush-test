...
describe('EditPortDrawer', () => {
  ...
  describe('Poe Schedule (base on Switch RBAC FF enabled)', () => {
    beforeEach(() => {
    ...
    describe('FF enabled', () => {
      beforeEach(() => {
        jest.mocked(useIsSplitOn).mockImplementation(
          ff => ff === Features.SWITCH_RBAC_API || ff === Features.SWITCH_SUPPORT_TIME_BASED_POE_TOGGLE
        )
      })
      ...
      describe('Single edit', () => {
        ...
        it('should render poe custom schedule correctly', async () => {
          render(<Provider>
            <EditPortDrawer
              visible={true}
              setDrawerVisible={jest.fn()}
          ...
          await waitForElementToBeRemoved(screen.queryAllByRole('img', { name: 'loader' }))
          expect(await screen.findByText('PoE Schedule')).toBeVisible()
          expect(await screen.findByTestId('poe-schedule-status')).toHaveTextContent('Custom Schedule')
        })
      })
      ...
      describe('Multiple edit', () => {
        ...
        it('should render multiple custom poe schedule correctly', async () => {
          ...
          await waitForElementToBeRemoved(screen.queryAllByRole('img', { name: 'loader' }))
          expect(await screen.findByText('PoE Schedule')).toBeVisible()
          expect(await screen.findByTestId('poe-multiple-schedule-status')).toHaveTextContent('Custom Schedule')
        })
        it('should render no poe schedule and custom poe schedule correctly', async () => {
          mockServer.use(
            rest.post(SwitchRbacUrlsInfo.getPortsSetting.url,
              (_, res, ctx) => res(ctx.json([{
                ...portSetting[0],
          ...
          await waitForElementToBeRemoved(screen.queryAllByRole('img', { name: 'loader' }))
          expect(await screen.findByText('PoE Schedule')).toBeVisible()
          expect(await screen.findByTestId('poe-multiple-schedule-status')).toHaveTextContent('Multiple values')
        })
      })
    })
  })
