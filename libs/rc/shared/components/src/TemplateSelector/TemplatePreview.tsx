...
export function TemplatePreview (props: TemplatePreviewProps) {
  ...
  const emailTemplateRequest = useGetTemplateByIdQuery({
    params: { templateScopeId: emailTemplateScopeId, templateId: templateGroup?.emailTemplateId } },
  { skip: !templateGroup })
  const smsTemplateRequest = useGetTemplateByIdQuery({
    params: { templateScopeId: smsTemplateScopeId, templateId: templateGroup?.smsTemplateId } },
  { skip: !templateGroup })
  ...
  const tabDetails: ContentSwitcherProps['tabDetails'] = [
    {
      ...
      value: 'email',
      children: <Loader style={{ height: 'auto', minHeight: 45 }} states={[emailTemplateRequest]}>
        ...
        <div dangerouslySetInnerHTML={{ __html:
              emailTemplateRequest.data?.messageTemplate ??
                $t({ defaultMessage: 'Template Not Found' }) }} />
      </Loader>
    },
    {
      label: $t({ defaultMessage: 'SMS' }),
      value: 'sms',
      children: <Loader style={{ height: 'auto', minHeight: 45 }} states={[smsTemplateRequest]}>
        <p>{smsTemplateRequest.data?.messageTemplate ??
          $t({ defaultMessage: 'Template Not Found' })}</p>
      </Loader>
    }]
