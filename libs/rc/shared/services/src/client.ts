...
import { convertToRelativeTime, formatter } from '@acx-ui/formatter'
import {
  Client,
  ClientUrlsInfo,
  CommonResult,
...
export const clientApi = baseClientApi.injectEndpoints({
  endpoints: (build) => ({
    getClients: build.query<TableResult<ClientInfo>, RequestPayload>({
      async queryFn (arg, _queryApi, _extraOptions, fetchWithBQ) {
        ...
        if (needSwitchData && clientList.totalCount > 0) {
          const uniqueClientApMacs: Set<string> = new Set()
          clientList.data.forEach((clientInfo) => {
            uniqueClientApMacs.add(clientInfo.apInformation.macAddress)
          })
          const switchClientMacs: string[] = Array.from(uniqueClientApMacs)
    ...
    getHistoryClientDetail: build.query<{ data: Client }, RequestPayload>({
      async queryFn (arg, _queryApi, _extraOptions, fetchWithBQ) {
        const { clientId, ...params } = arg.params as Record<string, string>
