...
export type { ProFormInstance as FormInstance }
...
export function StepsForm <FormValue = any> (
  ...
  const stepsFormRender: ProAntStepsFormProps['stepsFormRender'] = (form, submitter) => (
    <>
      ...
      <UI.ActionsContainer>
        <Space align='start' size={12}>{submitter}</Space>
      </UI.ActionsContainer>
    </>
  )
...
function StepForm <FormValue = any> (
  ...
  >
    ...
    {internalProps.state === 'finish' ? <UI.EditButton
      type='link'
      children={intl.getMessage('stepsForm.edit', 'Edit')}
      onClick={internalProps.onGotoStep}
    /> : null}
    <UI.StepIndicator state={internalProps.state} />
    {!internalProps.isLastStep ? <UI.StepLine /> : null}
