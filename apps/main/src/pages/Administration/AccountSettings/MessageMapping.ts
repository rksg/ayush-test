...
export const MessageMapping = {
  ...
  enable_r1_beta_access_description: defineMessage({ defaultMessage: 'This option enables current and future beta features for this account across all venues, networks, and users.{br} When disabled, any configuration related to beta features will remain available until deleted.' }),
  enable_r1_beta_disable_description: defineMessage({ defaultMessage: 'Please note that if you decide to disable beta features, your current configurations will be retained. However, you won\'t be able to modify them until you re-enable beta features.' }),
  ...
  enable_r1_beta_confirmation_msg: defineMessage({ defaultMessage: 'In order to enable the Beta features, we have to log you out. Once you log-in back, the features will be available for you to use. {br}{br2} Automatically logging out in {c} seconds…'})
}
