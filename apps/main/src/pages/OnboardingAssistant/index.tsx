...
export default function OnboardingAssistantModal (props: {
  visible: boolean,
  setVisible: (visible: boolean) => void,
  setAIChatVisible?: (visible: boolean) => void,
  initStep?: RuckusAiStepsEnum
}) {
  const { visible, setVisible, setAIChatVisible, initStep } = props
  const { $t } = useIntl()
  ...
  const [step, setStep] = useState(initStep || RuckusAiStepsEnum.WELCOME as RuckusAiStepsEnum)
  ...
  const closeModal = () => {
    ...
    setShowAlert(false)
    setStep(initStep || RuckusAiStepsEnum.WELCOME)
    setVisible(false)
    setCurrentStep(0)
