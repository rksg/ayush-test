...
describe('CaptiveNetworkForm-WISPr', () => {
  ...
  it('should test WISPr network successfully', async () => {
    ...
    fireEvent.blur(ips[0])
    fireEvent.change(ips[1], { target: { value: '2.3.3.3' } })
    fireEvent.blur(ips[1])
    fireEvent.change(ips[2], { target: { value: '2.3.3.2' } })
    ...
    fireEvent.blur(secrets[0])
    fireEvent.change(secrets[1], { target: { value: 'xxxxxxxxxx' } })
    fireEvent.blur(secrets[1])
    fireEvent.change(secrets[2], { target: { value: 'xxxxxxxxxx' } })
    fireEvent.blur(secrets[2])
    fireEvent.change(secrets[3], { target: { value: 'xxxxxxxxxx' } })
    fireEvent.blur(secrets[3])
