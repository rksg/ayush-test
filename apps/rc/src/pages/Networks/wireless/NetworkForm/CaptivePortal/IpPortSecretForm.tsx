...
export function IpPortSecretForm ({ serverType, order }:
  ...
  return (
    <>
      <Form.Item
        validateFirst
        name={['guestPortal','wisprPage',serverType, order, 'ip']}
      ...
      />
      <Form.Item
        name={['guestPortal','wisprPage',serverType, order, 'port']}
      ...
      />
      <Form.Item
        name={['guestPortal','wisprPage',serverType, order, 'sharedSecret']}
...
function checkIpAndPortUnique (
  ...
  const currentFieldSet = ['guestPortal','wisprPage',serverType, order]
  ...
  const ipList = relatedFields
    ...
    .map(filed => {
      const { ip, port } = getFieldValue(filed.slice(0, 4))
  ...
  if (hasDuplicate) {
    ...
    setFields([{
      name: ['guestPortal','wisprPage',serverType, order, relatedField],
