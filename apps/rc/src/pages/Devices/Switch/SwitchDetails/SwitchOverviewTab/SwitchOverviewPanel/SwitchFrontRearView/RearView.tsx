...
export function RearView (props:{
  ...
  return <>
    { (slot.powerStatus && switchModelInfo.powerSlots
      && slot.powerStatus.slotNumber <= switchModelInfo.powerSlots) ?
      <RearElement icon='P'
        status={slot.powerStatus.status}
        slotNumber={slot.powerStatus.slotNumber}
      /> : null
    }
    { (slot.fanStatus && switchModelInfo.fanSlots
      && slot.fanStatus.slotNumber <= switchModelInfo.fanSlots) ?
      <RearElement icon='F'
