...
import { useApListQuery, useGetApCapabilitiesQuery } from '@acx-ui/rc/services'
...
export function ApContextProvider (props: { children: ReactNode }) {
  ...
  const apCapabilitiesQuery = useGetApCapabilitiesQuery({ params: { serialNumber: (results.data?.[0].serialNumber ?? '') } })

  const { data } = results
  const values: Params<string> = {
    ...params,
    ...pick(data?.[0], fields) as Params<string>,
    ...({ capabilities: apCapabilitiesQuery.data } as Params<string>)
  }
  return <ApContext.Provider value={values}>
    <Loader states={[results, apCapabilitiesQuery]}>{
      data && data.length
        ? props.children
