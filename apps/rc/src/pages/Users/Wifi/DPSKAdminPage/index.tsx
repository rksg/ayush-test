...
export default function DPSKAdminPage () {
  const { $t } = useIntl()
  const { data: userProfile } = useUserProfileContext()
  const basePath = useTenantLink('/users/dpskAdmin')
  ...
  useEffect(() => {
    return () => {
      if(userProfile && userProfile.role === RolesEnum.DPSK_ADMIN) {
        navigate({
          ...basePath,
