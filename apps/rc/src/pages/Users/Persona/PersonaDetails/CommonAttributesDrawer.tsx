...
export function CommonAttributesDrawer (props:{persona:Persona,  externalData: ExternalIdentity, visible: boolean, onClose: ()=>void}) {
    ...
    return <Drawer
        ...
        children={<>
            <Descriptions>
                ...
                <Descriptions.Item
                    key={'displayName'}
                    label={$t({defaultMessage: 'DisplayName'})}
            ...
            <Descriptions>
                <Descriptions.Item
                    ...
                    children={<div>{externalData.groups?.map(group => <p>{group}</p>)}</div>}
                />
                <Descriptions.Item
                    key={'role'}
                    label={$t({defaultMessage: 'Role'})}
                    children={<div>{externalData.roles?.map(role => <p>{role}</p>)}</div>}
                />
            </Descriptions>
            <Divider/>
            <Descriptions>
