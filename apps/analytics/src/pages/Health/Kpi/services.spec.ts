...
describe('Services for health kpis', () => {
  ...
  describe('Timeseries', () => {
    ...
    it('should return correct data for fetchThresholdPermission query', async () => {
      ...
      const { status, data, error } = await store.dispatch(
        thresholdApi.endpoints.fetchThresholdPermission.initiate({
    ...
    it('should return correct data for saveThreshold mutation', async () => {
      ...
      const response = await store.dispatch(
        thresholdApi.endpoints.saveThreshold.initiate({
          path: validPayload.path,
          name: 'apCapacity',
          value: 30
        })
  ...
  describe('Get Thresholds', () => {
    ...
    it('should return correct data', async () => {
      ...
      const { status, data, error } = await store.dispatch(
        thresholdApi.endpoints.getKpiThresholds.initiate(props)
      )
      expect(status).toBe('fulfilled')
